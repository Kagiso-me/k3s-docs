...
* LAN-only exposure
* Works fine on your kubectl/Prometheus/Grafana RPi

---

# ADDITION:

# 14. File-Based Auto-Unseal for Vault (Pi Local Setup)

To ensure Vault can automatically unseal on startup and your snapshot + rsync cronjob runs without manual intervention, we will use **file-based auto-unseal**.

---

## 1️⃣ Create secure unseal key file

```bash
# Create directory for unseal key
sudo mkdir -p /srv/vault/unseal
sudo chown vault:vault /srv/vault/unseal
sudo chmod 700 /srv/vault/unseal

# Generate a random 32-byte key and write to file using sudo tee
sudo head -c 32 /dev/urandom | base64 | sudo tee /srv/vault/unseal/unseal.key > /dev/null

# Set correct ownership and permissions
sudo chown vault:vault /srv/vault/unseal/unseal.key
sudo chmod 600 /srv/vault/unseal/unseal.key

# Verify the key file
ls -l /srv/vault/unseal/unseal.key
```

* `head -c 32 /dev/urandom | base64` → generates a secure random key
* `tee` allows writing to a directory requiring root permissions
* `chmod 600` ensures only Vault can read the key

---

## 2️⃣ Update Vault configuration

Edit `/etc/vault.d/vault.hcl` and add the `seal` block:

```hcl
seal "file" {
  path = "/srv/vault/unseal/unseal.key"
}
```

Full example configuration with auto-unseal:

```hcl
ui = true
api_addr = "http://0.0.0.0:8200"
cluster_addr = "http://<YOUR_RPI_IP>:8201"
...
* Replace `<YOUR_RPI_IP>` with your Pi’s LAN IP
* `seal "file"` points to the secure key we just created

---

## 3️⃣ Restart Vault and verify
...
* `Sealed` should now show `false` automatically
* Vault is ready to accept snapshot commands

---

## 4️⃣ Test snapshot with auto-unseal

```bash
vault operator raft snapshot save /srv/vault/snapshots/test.snap
```

* Should succeed without manually unsealing
* The cronjob `/usr/local/bin/vault-snapshot.sh` will now run unattended

---

## 5️⃣ Backup the unseal key

```bash
rsync -avz --no-perms /srv/vault/unseal/unseal.key kagiso@10.0.10.80:/mnt/tera/backups/vault/
```

* Keep a secure copy in TrueNAS in case of Pi failure
* Without this key, you **cannot recover Vault data**

---

With this setup:

* Vault auto-unseals at startup
* Snapshots can run automatically via cron
* Rsync to TrueNAS works without errors
* Single-node homelab setup is fully self-contained
